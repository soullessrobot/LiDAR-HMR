...
from ._gcnn import GraphConvolution, GraphResBlock, DynamicGraphResBlock
...
class GraphormerLayer(nn.Module):
    def __init__(self, config, adj_mat = None):
        ...
        if self.has_graph_conv == True:
            self.graph_conv = GraphResBlock(config.hidden_size, config.hidden_size, mesh_type=self.mesh_type, adj_mat=adj_mat)
        self.num_mesh_points = adj_mat.shape[0] if adj_mat is not None else 431
    ...
    def MHA_GCN(self, hidden_states, attention_mask, head_mask=None,
        ...
        if self.has_graph_conv==True:
            if self.mesh_type == 'body':
                joints = attention_output[:,0:0,:] 
                vertices = attention_output[:,0:self.num_mesh_points,:]
                img_tokens = attention_output[:,self.num_mesh_points:,:]
...
class GraphormerEncoder(nn.Module):
    def __init__(self, config, adj_mat = None):
        ...
        self.layer = nn.ModuleList([GraphormerLayer(config, adj_mat=adj_mat) for _ in range(config.num_hidden_layers)])
...
class EncoderBlock(BertPreTrainedModel):
    def __init__(self, config, adj_mat = None):
        ...
        self.encoder = GraphormerEncoder(config, adj_mat = adj_mat)
...
class Graphormer(BertPreTrainedModel):
    ...
    def __init__(self, config, adj_mat = None):
        ...
        self.config = config
        self.bert = EncoderBlock(config, adj_mat = adj_mat)
...
class DynamicLayer(nn.Module):
    def __init__(self, config):
        ...
        if self.has_graph_conv == True:
            self.graph_conv = DynamicGraphResBlock(config.hidden_size, config.hidden_size, config.graph_norm)
        
        self.intermediate = BertIntermediate(config)
        self.output = BertOutput(config)

    def MHA_GCN(self, hidden_states, attention_mask, head_mask=None,
                history_state=None, seg = None):
        attention_outputs = self.attention(hidden_states, attention_mask,
                head_mask, history_state)
        attention_output = attention_outputs[0]

        if self.has_graph_conv==True:
            joints_vertices = self.graph_conv(attention_output, seg = seg)
        else:
            joints_vertices = attention_output
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None,
                history_state=None, seg = None):
        return self.MHA_GCN(hidden_states, attention_mask, head_mask,history_state, seg = seg)
...
class DynamicEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.output_attentions = config.output_attentions
        self.output_hidden_states = config.output_hidden_states
        self.layer = nn.ModuleList([DynamicLayer(config) for _ in range(config.num_hidden_layers)])

    def forward(self, hidden_states, attention_mask, head_mask=None,
                encoder_history_states=None, seg = None):
        all_hidden_states = ()
        all_attentions = ()
        for i, layer_module in enumerate(self.layer):
            if self.output_hidden_states:
...
class DyEncoderBlock(BertPreTrainedModel):
    def __init__(self, config):
        super(DyEncoderBlock, self).__init__(config)
        self.config = config
        #self.embeddings = BertEmbeddings(config)
        ...
        self.pcd_embedding = nn.Linear(self.img_dim, self.config.hidden_size, bias=True)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        if self.use_img_layernorm:
            self.LayerNorm = LayerNormClass(config.hidden_size, eps=config.img_layer_norm_eps)
    ...
    def forward(self, img_feats, input_ids=None, token_type_ids=None, attention_mask=None,
            position_ids=None, head_mask=None, seg = None):
        #print(img_feats.shape)
        batch_size = len(img_feats)
        seq_length = len(img_feats[0])
        input_ids = torch.zeros([batch_size, seq_length],dtype=torch.long).cuda()
        ...
        # Project input token features to have spcified hidden size
        img_embedding_output = self.pcd_embedding(img_feats)
        ...
        encoder_outputs = self.encoder(embeddings,
                extended_attention_mask, head_mask=head_mask, seg = seg)
        sequence_output = encoder_outputs[0]
...
class DynamicGraphNet(BertPreTrainedModel):
    '''
    The archtecture of a transformer encoder block we used in Graphormer
    '''
    def __init__(self, config):
        super(DynamicGraphNet, self).__init__(config)
        self.config = config
        self.bert = DyEncoderBlock(config)
        self.cls_head = nn.Linear(config.hidden_size, self.config.output_feature_dim)
        self.residual = nn.Linear(config.img_feature_dim, self.config.output_feature_dim)
        self.apply(self.init_weights)

    def forward(self, img_feats, seg, input_ids=None, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
            next_sentence_label=None, position_ids=None, head_mask=None):
        '''
        # self.bert has three outputs
        # predictions[0]: output tokens
        ...
        '''
        predictions = self.bert(img_feats=img_feats, input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids,
                            attention_mask=attention_mask, head_mask=head_mask, seg = seg)
