...
class FaceAreaLoss(nn.Module):
    def __init__(self, face):
        super(FaceAreaLoss, self).__init__()
        self.face = face

    def forward(self, coord_out, coord_gt):
        face = torch.LongTensor(self.face).cuda()

        d1_out = (coord_out[:, face[:, 1], :] - coord_out[:, face[:, 0], :])
        d2_out = (coord_out[:, face[:, 2], :] - coord_out[:, face[:, 0], :])
        area = torch.cross(d1_out, d2_out).norm(dim = -1) / 2
        # import pdb; pdb.set_trace()
        d1_gt = (coord_gt[:, face[:, 1], :] - coord_gt[:, face[:, 0], :])
        d2_gt = (coord_gt[:, face[:, 2], :] - coord_gt[:, face[:, 0], :])
        area_gt = torch.cross(d1_gt, d2_gt).norm(dim = -1) / 2
        loss = torch.abs(area - area_gt).mean()
        return loss

def get_loss(faces):
    loss = CoordLoss(has_valid=False), NormalVectorLoss(faces), EdgeLengthLoss(faces), CoordLoss(has_valid=False), CoordLoss(has_valid=False), FaceAreaLoss(faces)
