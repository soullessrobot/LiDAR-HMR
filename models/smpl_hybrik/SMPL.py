...
from .lbs import lbs, hybrik, rotmat_to_quat, quat_to_rotmat, meshik
...
class SMPL_layer(nn.Module):
    ...
    def __init__(self,
        ...
        if h36m_jregressor is not None:
            self.register_buffer(
                'J_regressor_h36m',
        ...
        with open(indx_vert_path, 'rb') as indx_vert:
            indx_vert = pk.load(indx_vert)
        self.indx_vert = [indx_vert[jn] for jn in self.JOINT_NAMES[:24]]
        # self.register_buffer('indx_vert', indx_vert)
    ...
    def meshik(
        ...
        vertices, new_joints, rot_mats, trans = meshik(
            betas, pred_verts,
            self.v_template, self.shapedirs, self.posedirs,
            self.J_regressor, self.parents, self.children_map,
            self.lbs_weights, indx_vert = self.indx_vert, dtype=self.dtype, train=self.training)

        rot_mats = rot_mats.reshape(batch_size * 24, 3, 3)
        theta = matrix_to_axis_angle(rot_mats)
        # rot_mats = rotmat_to_quat(rot_mats).reshape(batch_size, 24 * 4)
        output = ModelOutput(theta = theta.reshape(batch_size, -1).to(device), trans = trans.to(device), betas = betas.to(device),
            vertices=vertices, rot_mats=rot_mats)
        return output
