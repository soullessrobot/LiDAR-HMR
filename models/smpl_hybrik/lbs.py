...
def meshik(betas, pred_verts, v_template, shapedirs, posedirs, J_regressor, parents, children,
           lbs_weights, indx_vert, dtype=torch.float32, train=False, leaf_thetas=None, naive=False):
    batch_size = max(betas.shape[0], pred_verts.shape[0])
    device = betas.device
    ...
    # NxJx3 array
    rest_J = vertices2joints(J_regressor, v_shaped)
    pose_skeleton = vertices2joints(J_regressor, pred_verts)
    trans_diff = pose_skeleton[:,[0]] - rest_J[:,[0]]
    ################# updated 20240410
    # pred_verts = pred_verts - trans_diff
    #################
    ...
    rot_mat_chain = [global_orient_mat] #累乘
    rot_mat_local = [global_orient_mat] #单个pose
    v_posed = v_shaped.clone()
    
    joints_J = torch.unsqueeze(rest_J, dim=-1)
    rel_joints = joints_J.clone()
    rel_joints[:, 1:] -= rel_joints[:, parents[1:]].clone()
    rel_rot_mat_rest = [transform_mat(
            global_orient_mat.reshape(-1, 3, 3),
            rel_joints[:,0].reshape(-1, 3, 1)).reshape(batch_size, 4, 4)]
    A_rest = torch.zeros([batch_size, 24, 4, 4])
    joints_homogen = F.pad(joints_J, [0, 0, 0, 1])
    W = lbs_weights.unsqueeze(dim=0).expand([batch_size, -1, -1])
    
    for i in range(1, parents.shape[0]):
        local_vert = pred_verts[:, indx_vert[i]] - pose_skeleton[:,[i]]
        local_vert_t_pose = v_posed[:, indx_vert[i]] - rest_J[:,[i]] #[B,N,3]
        
        n_verts = local_vert_t_pose.shape[1]
        R_parents = rot_mat_chain[parents[i]]
        M_ = rest_J[:, i] #[B,3]
        W_this = W[:,indx_vert[i]] #[B,N,24]
        wA = torch.matmul(W_this, A_rest.view(batch_size, 24, 16).to(device)).view(batch_size, -1, 4, 4) #[B,N,4,4]
        t_p = rel_joints[:,i]
        W_tj = W_this[...,i] #[B,N]
        
        wt = W_tj.unsqueeze(2) * local_vert_t_pose
        wAt = wA[...,:3,:3] @ local_vert_t_pose.unsqueeze(3) #[B,N,3,1]
        RwAt = R_parents.unsqueeze(1) @ wAt #[B,N,3,1]
        T0 = wt #local_vert_t_pose
        P0 = local_vert - RwAt.squeeze()
        
        S = T0.transpose(1, 2).bmm(P0) #[B,3,3]
        ...
        rot_mat = R_parents.permute(0,2,1) @ rot_mat
        ...
        transforms_mat = transform_mat(
            rot_mat.reshape(-1, 3, 3),
            t_p).reshape(batch_size, 4, 4)
        rel_rot_mat_this = torch.matmul(rel_rot_mat_rest[parents[i]], transforms_mat)
        rel_rot_mat_rest.append(rel_rot_mat_this)
        
        rel_transforms = rel_rot_mat_this.clone()
        rel_transforms[:,:3,3] -= torch.matmul(rel_rot_mat_this, joints_homogen[:,i])[:,:3,0]
        A_rest[:,i] = rel_transforms
    # (B, K + 1, 3, 3)
    ...
    verts = v_homo[:, :, :3, 0]
    verts += trans_diff
    J_transformed += trans_diff

    return verts, J_transformed, rot_mats, trans_diff.squeeze(1) #, J_from_verts_h36m

def batch_get_this_orient_using_verts(t_this, R_parents, local_verts, local_vert_t_pose, parents, children, dtype):
    k = R_parents[:3,3]
    r_ = R_parents[:3,:3]
    m = t_this
    T0 = m - local_vert_t_pose
    P0 = local_verts - k
    S = T0.bmm(P0.transpose(1, 2))
...
def batch_rigid_transform(rot_mats, joints, parents, dtype=torch.float32):
    ...
    rel_transforms = transforms.clone() - F.pad(
        torch.matmul(transforms.clone(), joints_homogen), [3, 0, 0, 0, 0, 0, 0, 0])
