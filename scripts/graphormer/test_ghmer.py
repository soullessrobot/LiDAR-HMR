...
#                     epoch, i, len(dataloader), loss_all=loss_all, loss_vert=loss_vert, loss_joint = loss_joint)
#             print(msg)
#             # logger.info(msg)
...
def run_validate(val_loader, Graphormer_model, smpl, mesh_sampler, device = 0, show = False):
    ...
    with torch.no_grad():
        for i, sample in tqdm(enumerate(val_loader)):
            ...
            gt_smpl_3d_joints = gt_smpl_3d_joints[:,H36M_J17_TO_J14,:] 
            ...
            pred_3d_joints_from_smpl = pred_3d_joints_from_smpl[:,H36M_J17_TO_J14,:]
            ...
            # print(error_relative_edges.item(), int(torch.sum(has_3d_joints)))
            mPERE += error_relative_edges.item()
            count_num += 1
            # seg_ = ret_dict['seg']
            ...
            if show:
                ...
                ax1.plot_trisurf(pred_show[0,:,0], pred_show[0,:,1], pred_show[0,:,2], triangles=model.smpl.faces)
                ax2.plot_trisurf(gt_show[0,:,0], gt_show[0,:,1], gt_show[0,:,2], triangles=model.smpl.faces)
    ...
    mpere = mPERE / count_num
    # mpere = sum(mpere)
    # precision = all_gather(float(Precision.avg))
    # precision = sum(precision)/len(precision)

    return val_mPVE, val_mPJPE, val_PAmPJPE, val_count, mpere
...
val_mPVE, val_mPJPE, val_PAmPJPE, val_count, mPERE = run_validate(test_loader, model, model.smpl, model.mesh, show = show)
print('mPVE: '+str(val_mPVE) + '; MPJPE:' + str(val_mPJPE) + '; PA-MPJPE:' + str(val_PAmPJPE))
print('mPERE: '+str(mPERE))