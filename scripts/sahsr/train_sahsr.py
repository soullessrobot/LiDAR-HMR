...
from scripts.eval_utils import mean_per_vertex_error, setup_seed
...
for epoch in range(50):
    ...
    mpjpe, mpvpe = test(model, test_loader)
    os.makedirs(save_dir, exist_ok=True)
    msg = 'epoch{0}_{mpjpe:.5f}_{mpvpe:.5f}.pth'.format(epoch, mpjpe = mpjpe, mpvpe = mpvpe)
    torch.save(state, os.path.join(save_dir, msg))
    print('MPJPE: '+str(mpjpe) + '; MPVPE:'+str(mpvpe) + ';')
