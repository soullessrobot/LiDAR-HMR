...
model_type = 'pct_p2m_svd_e2e' #'lpformer', 'v2v'
...
model = pose_mesh_net_svd(train_p2m=True).cuda()
bs = 32 #64
train_loader = DataLoader(train_dataset, batch_size=bs, shuffle=True)
test_loader = DataLoader(test_dataset, batch_size=bs, shuffle=False)
...
if args.p2m_state_dict != '':
    ...
    # import pdb; pdb.set_trace()
    # optimizer.load_state_dict(state_dict['optimizer']) #report error
    epoch_now = 29
...
for epoch in range(epoch_now+1, 50):
    if epoch == 40:
