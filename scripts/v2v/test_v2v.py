...
def test(model, dataloader):
    ...
    for sample in tqdm(dataloader):
        ...
        with torch.no_grad():
            ret_dict = model(pcd.cuda(), center.cuda())
...
def test_ap(model, dataloader):
    model.eval()
    mpjpe = 0
    number = 0
    precision = 0
    ap = np.zeros([4])# 0.075 0.10 0.125 0.150
    ...
    for sample in tqdm(dataloader):
        ...
        if args.dataset == 'lidarh26m':
            xyz -= xyz[:,[0]]
            gt_xyz -= gt_xyz[:,[0]]
        diff = (xyz - gt_xyz.to(xyz.device)).norm(dim = -1)
        ap[0] += (diff < 0.075).sum().item() / diff.numel()
        ap[1] += (diff < 0.10).sum().item() / diff.numel()
        ap[2] += (diff < 0.125).sum().item() / diff.numel()
        ap[3] += (diff < 0.15).sum().item() / diff.numel()
        
        mpjpe += diff.mean() if flag is None \
              else (diff * flag).sum() / flag.sum()
        # vis_result = vis
        # vis_tp = vis_result * gt_vis.cuda()
        # precision += torch.sum(vis_tp) / torch.sum(vis_result)
        number += 1
...
# optimizer = optim.AdamW(model.parameters(), lr = 2.5e-4, betas = (0.85, 0.95)) #
